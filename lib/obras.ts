...
export async function getObraPorSlug(slug: string) {
  ...
  const { data } = await supabase
    .from("obras")
    .select(
      "id, slug, nombre, ubicacion, estado, fecha_inicio, fecha_fin_estimada, presupuesto, valor_m2_usd, domicilio, unidades_funcionales, pisos, sup_construccion_m2, sup_venta_m2, lote_valor_usd, lote_superficie_m2, lote_vendedor, lote_detalle"
    )
    .eq("slug", slug)
    .maybeSingle();
